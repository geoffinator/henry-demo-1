...
function WeeklySummary({ liveJobs }) {
  const jobs = liveJobs || JOBS;
  const weekJobs = jobs.filter(j => j.date && j.date >= "2026-03-24" && j.date <= "2026-03-30");
...
const CUSTOMER_PORTAL_SYSTEM = `You are the customer-facing chat agent for Henry the Exterminator, a licensed pest control company in the Greater Toronto Area.

Your job is to respond as the OS would respond to a real customer inquiry. Be warm, direct, and professional. Sound like a real local business — not a bot.

Rules:
- If the customer gives their name, address, and phone or email, confirm a booking with a specific technician (Daniel, Bruce, or Maria), date, and time.
- If info is missing, ask for all missing pieces in one message.
- Provide pricing when asked: $130–$160 ants/wasps, $150–$250 mice/cockroaches, $300–$450 bed bugs, $300–$450+ commercial.
- Keep responses short and natural.
- Never use filler like "Absolutely!" or "Great question!".

Respond with ONLY valid JSON:
{
  "reply": "Your message to the customer",
  "bookingConfirmed": true or false,
  "extractedName": "customer name or null",
  "extractedAddress": "address or null",
  "extractedPest": "pest type or null",
  "newJobSummary": "one-line summary for the owner's D1 inbox if booking confirmed, else null"
}`;
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch("/api/proxy", {
        method:"POST", headers:{ "Content-Type":"application/json" },
        body:JSON.stringify({
          model:"claude-haiku-4-5", max_tokens:600, system:CUSTOMER_PORTAL_SYSTEM,
      ...
      if (parsed.bookingConfirmed && !bookingDone) {
        ...
        const newJob = {
          ...
          conversation: updatedConvo.map((m,i) => ({
            from: m.from === "customer" ? "customer" : "os",
            time: "Mar 31, Live Demo",
            msg: m.text
          })),
          timeline:[
            { time:"Mar 31, Live Demo", event:"Lead received", detail:"Inbound via customer portal — live demo", type:"lead" },
            { time:"Mar 31, Live Demo", event:"OS responded",  detail:"Intake completed automatically", type:"os" },
            { time:"Mar 31, Live Demo", event:"Job booked",    detail:`Daniel assigned — ${parsed.newJobSummary||"Booked"}`, type:"booked" },
          ]
        };
